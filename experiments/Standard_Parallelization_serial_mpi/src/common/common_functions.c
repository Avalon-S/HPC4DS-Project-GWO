...
int partition(Wolf *population, int low, int high)
{
    double pivot = population[high].fitness; // Choose the last element as the pivot
    int i = low - 1;
    ...
    for (int j = low; j < high; j++)
    {
        if (population[j].fitness <= pivot)
        {
            ...
            Wolf temp = population[i];
            population[i] = population[j];
            population[j] = temp;
        }
    }

    // Swap population[i + 1] and population[high] (pivot)
    Wolf temp = population[i + 1];
...
// Quicksort function
void quick_sort(Wolf *population, int low, int high)
{
    if (low < high)
    {
...
{
    quick_sort(population, 0, pop_size - 1);
}
