...
log_dir="$project_root/logs/parallel/1024/2_4"  
mkdir -p $log_dir
...
for func_id in F1 F2 F3; do
    mpiexec -np 2 "$bin_dir/GWO_hybrid_${dim}" $func_id $dim $cores         > "$log_dir/Hybrid_${func_id}_output.log" 2> "$log_dir/Hybrid_${func_id}_error.log"

    mpiexec -np 2 "$bin_dir/HGT_GWO_hybrid_${dim}" $func_id $dim $cores         > "$log_dir/Hybrid_${func_id}_output.log" 2> "$log_dir/Hybrid_${func_id}_error.log"
done
