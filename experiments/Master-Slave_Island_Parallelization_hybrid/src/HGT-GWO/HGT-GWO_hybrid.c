...
int main(int argc, char *argv[])
{
    ...
    if (argc < 5) {
        if (rank == 0) {
            fprintf(stderr, "Usage: %s <test_function_name> <dimension> <num_cores> <num_threads>\n", argv[0]);
        }
    ...
    if (g_dimension <= 0 || num_cores <= 0 || num_threads <= 0 || num_cores != size) {
        if (rank == 0) {
    ...
    char core_info[64];
    snprintf(core_info, sizeof(core_info), "%d_cores/%d_threads", num_cores, num_threads);
    ...
    // Distribute the initial population
