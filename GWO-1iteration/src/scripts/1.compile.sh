...
for dim in 256 512 1024; do
    gcc -std=c99 -o "$output_dir/GWO_serial_${dim}" \
    ...
    gcc -std=c99 -o "$output_dir/HGT_GWO_serial_${dim}" \
...
# Compile MPI+OpenMP(hybrid) version
for dim in 256 512 1024; do
    # Compile GWO parallel version
    mpicc -std=c99 -fopenmp -o "$output_dir/GWO_hybrid_${dim}" \
        "$project_root/src/GWO/GWO_hybrid.c" \
        "$project_root/src/common/common_functions.c" \
        "$project_root/src/common/test_functions.c" -lm \
        > "$log_dir/GWO_hybrid_${dim}.out" 2> "$log_dir/GWO_hybrid_${dim}.err"
    echo "Compiled Hybrid binary for dim=${dim} at: $output_dir/GWO_hybrid_${dim}"

    # Compile HGT-GWO hybrid version
    mpicc -std=c99 -fopenmp -o "$output_dir/HGT_GWO_hybrid_${dim}" \
        "$project_root/src/HGT-GWO/HGT-GWO_hybrid.c" \
        "$project_root/src/common/common_functions.c" \
        "$project_root/src/common/test_functions.c" -lm \
        > "$log_dir/HGT_GWO_hybrid_${dim}.out" 2> "$log_dir/HGT_GWO_hybrid_${dim}.err"
    echo "Compiled HGT-GWO Hybrid binary for dim=${dim} at: $output_dir/HGT_GWO_hybrid_${dim}"
done
